...
import { SettingsItem } from "./types/Product";

export const getSettingsNameByIndex: (
  index: number,
  type: "milk" | "sizes" | "ingredients",
  settings: {
    milk: SettingsItem[];
    sizes: SettingsItem[];
    ingredients: SettingsItem[];
  }
) => string = (index, type, settings) => {
  return settings[type][index].name;
};

export const getSort = (sort: number) =>
  sort === 0 ? "rating" : sort === 1 ? "title" : "price";
export const getCategory = (category: number) =>
  category === 0 ? "1|2" : category;
...
export const getProductsAmountByType = (products: SelectedProduct[] | []) => {
  ...
  const snaksLabel =
    totalAmount[1] === 0 || totalAmount[1] > 4
      ? "снэков"
      : totalAmount[1] === 1
      ? "снэк"
      : "снэка";
  ...
  return [
    `${totalAmount[0]} ${drinksLabel}`,
    `${totalAmount[1]} ${snaksLabel}`,
  ];
};
...
export const getValidCitiesList: (
  ...
  cities.forEach((city, index) =>
    validCityList.push({ id: index, text: city.text })
  );
...
export const getValidAddressesList: (
  ...
  addresses.forEach((address, index) =>
    validCityList.push({ id: index, text: address.text })
  );

  return validCityList;
};

export const getCityIdByIndex: (index: number, cities: City[]) => string = (
  index,
  cities
) => {
